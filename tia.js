...
async function performTransaction(walletInfo, numberOfTimes) {
    ...
    const gasPrice = GasPrice.fromString("0.025uinj");
    const wallet = await DirectSecp256k1Wallet.fromKey(Buffer.from(walletInfo.privateKey, "hex"), "injective");
    const client = await SigningStargateClient.connectWithSigner(rpcEndpoint, wallet, { gasPrice: gasPrice });
    const fee = {
        amount: coins(400, "uinj"),
        gas: "110000",
    };
    for (let i = 0; i < numberOfTimes; i++) {
        try {
            const [account] = await wallet.getAccounts();
            const amount = coins(1, "uinj");
